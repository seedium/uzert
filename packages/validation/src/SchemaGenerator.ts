...
import SwaggerOptionsNotSetError from './errors/SwaggerOptionsNotSetError';
import SchemaGeneratorMerge from './errors/SchemaGeneratorMerge';
...
export default class SchemaGenerator {
  ...
  public async generateSwaggerDoc() {
    const docPath = `${this.swaggerAbsoluteFSPath}/${SchemaGenerator.docFileName}`;
    const indexFilePath = `${this.swaggerAbsoluteFSPath}/index.html`;
    ...
    fs.writeFileSync(
      ...
      fs
        ...
        .replace(
          /url: ".*"/,
          `url: "${this._prefix || ''}/${SchemaGenerator.docFileName}"`,
        ),
    );
  }
  ...
  public mergeSchema(schema: any) {
    if (!schema.$merge.source.$ref) {
      throw new SchemaGeneratorMerge();
    }
