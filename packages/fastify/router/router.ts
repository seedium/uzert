...
export class Router {
  ...
  constructor(private readonly _container: UzertContainer, private readonly _app: fastify.FastifyInstance) {
    this._containerScanner = new ContainerScanner(_container);
  }
  public post(path: string, handler: fastify.RequestHandler) {
    this.route(path, 'POST', handler);
  }
  public get(path: string, handler: fastify.RequestHandler) {
    this.route(path, 'GET', handler);
  }
  public put(path: string, handler: fastify.RequestHandler) {
    this.route(path, 'PUT', handler);
  }
  public delete(path: string, handler: fastify.RequestHandler) {
    this.route(path, 'DELETE', handler);
  }
  public patch(path: string, handler: fastify.RequestHandler) {
    this.route(path, 'PATCH', handler);
  }
  public head(path: string, handler: fastify.RequestHandler) {
    this.route(path, 'HEAD', handler);
  }
  protected route(path, method: fastify.HTTPMethod, handler: fastify.RequestHandler) {
    ...
    this._app.route({
      ...
      method,
      url: path,
      handler: this.bindMethod<fastify.RequestHandler>(handler, instance),
    });
  }
  protected exploreMethod(handler: fastify.RequestHandler): fastify.RouteShorthandOptions {
  ...
  protected resolvePipes(handler: fastify.RequestHandler): Pipe[] {
  ...
  protected reflectInstance(handler: fastify.RequestHandler): any {
  ...
  protected reflectPipes(handler: fastify.RequestHandler): Type<Pipe>[] {
    return Reflect.getMetadata(PIPES_METADATA, handler) || [];
  }
  protected reflectRouterOptions(handler: fastify.RequestHandler): fastify.RouteShorthandOptions {
    return Reflect.getMetadata(ROUTER_OPTIONS, handler) || {};
  }
  protected applyHandlers(
    pipes: Pipe[],
    preHandlers: fastify.FastifyMiddleware[] | fastify.FastifyMiddleware = [],
  ...
  protected getPipesHandlers(pipes: Pipe[]): fastify.FastifyMiddleware[] {
