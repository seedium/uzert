...
export class InstanceLoader {
  ...
  private createPrototypesOfControllers(module: Module) {
    ...
    controllers.forEach((wrapper) => this.injector.loadPrototype<IInjectable>(wrapper, controllers));
  }
  ...
  private async createInstancesOfRoutes(module: Module) {
    ...
    await Promise.all(wrappers.map((item) => this.injector.loadRoute(item, module)));
  }
